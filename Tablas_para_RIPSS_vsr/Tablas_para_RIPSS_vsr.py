...
def crea_tabla_16(nombre_eps):
    ...
    for row in sheet_capacidad_recursos.iter_rows(min_row=2, values_only=True):
        ...
        if codigo_habilitacion in set_ips:
            ...
            cantidad = str(row[21]).strip()
            registros = diccionario_ips_recursos.get(codigo_habilitacion,[]) 
            registros.append([sede, coca_codigo, cantidad])
            diccionario_ips_recursos[codigo_habilitacion] = registros
    ...
    for servicio in sorted(list(servicios)):
        ...
        registro_recurso = diccionario_ips_recursos.get(servicio,[])
        cantidad = 0
        tot_cantidad = 0
        for recurso in registro_recurso:
            coca_codigo = recurso[1]
            cantidad = recurso[2]
            tot_cantidad += cantidad 
        if servicio in lista_servicios:
            data.append(["INTERNACION",servicio,
